...
static void _Window_windowsize_callback(GLFWwindow* window, int width, int height) {
  Window* win = static_cast<Window*>(glfwGetWindowUserPointer(window));
  if (win->listener()) {
...
Window::Window(const WindowOptions& opt) {
  ...
  _window = glfwCreateWindow(
      (int)(opt.width * xscale),
      (int)(opt.height * xscale),
  ...
  glfwSetCharCallback(_window, Window_char_callback);
  glfwSetCursorPosCallback(_window, Window_cursor_pos_callback);
  glfwSetMouseButtonCallback(_window, Window_mouse_button_callback);
  glfwSetScrollCallback(_window, Window_scroll_callback);
  glfwSetWindowSizeCallback(_window, _Window_windowsize_callback);
  ...
  GLenum err = glewInit();
  if (GLEW_OK != err) {
    cerr << "[Error] GLEW failed to initialize. " << (const char*)glewGetErrorString(err) << endl;
    exit(1);
  }
  ...
  glfwSwapInterval(opt.enable_vsync ? 1 : 0);
}
